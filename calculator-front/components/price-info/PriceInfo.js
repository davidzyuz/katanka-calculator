...
function PriceWithDiscount(props) {
  const [value, setValue] = useState(Number(props.firstVar) * 100);
  console.log('firstvar', typeof Number(props.firstVar));
  return (
    <div>
      {props.bn} -
      <input type="text" name="firstVar" value={Number(props.firstVar) * 100} />
      = {props.cash}
    </div>
  )
}
